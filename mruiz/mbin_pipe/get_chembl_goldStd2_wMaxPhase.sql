SELECT   td.chembl_id, md.chembl_id, MIN(pp.accession), MIN(pp.description), MIN(do.source_domain_id), MIN(act.standard_value),AVG(act.standard_value) , MAX(act.standard_value), MIN(ms.synonyms), MIN(canonical_smiles),MIN(pp.organism), MAX(md.max_phase) -- por alguna razon esto arroja error 
FROM assays
