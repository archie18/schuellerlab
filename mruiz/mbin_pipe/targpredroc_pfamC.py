...
def main():
    ...
    for idx,filename in enumerate(filenames):
        ...
        #print (chemb)
        if 'fil' in filename:
            cutof = re.search(r"C(\d+)",filename)
            #print (str(cutof.group()))
            if 'fp2' in filename:
                f_toPrint = str(cutof.group())
                #print (f_toPrint)
            elif 'mold' in filename:
                f_toPrint = "ChEMBL" + chemb + " Mold2 Tc=" + str(cutof.group())
                #print (f_toPrint)
        else:
            if 'fp2' and 'C0' in filename:
                f_toPrint = "Tc=0.99 FULL"
                #print (f_toPrint)
            elif 'mold' in filename:
                f_toPrint = "ChEMBL" + chemb + " Mold2 FULL"     
                #print (f_toPrint)
        roc(y_true, y_pred, f_toPrint, fig_filename, total_y_true, len(y_true) - total_y_true, colour[count])
        precision_recall(y_true, y_pred, f_toPrint, fig_filename, colour[count])
