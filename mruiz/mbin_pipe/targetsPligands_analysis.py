...
def targ_ligs_dict(out):
    ...
    with open(out, 'r') as file:
            for line in file:
                ...
                if yt == 1 and yp != -99.0:
                    ligid = tokens[1]
                    grouped[ligid].add(targid)
    return(grouped)

def readFile(out,lig_dic,cutoff):
    x_arrayL = []
    tanis_arrayL = []
    with open(out, 'r') as f:
        for line in f:
            ...
            if yp>=cutoff and yp != -99.0:
                targets_Q = str(len(lig_dic[ligid]))
                print(ligid+"\t"+targets_Q)
                x_arrayL.append(targets_Q)
...
def main():
    ...
    lig_dic = targ_ligs_dict(out)
    readFile(out, lig_dic, cutoff)
