    ...
    for idx,filename in enumerate(filenames):
        ...
        with open(filename, 'r') as file:
            for line in file:
                ...
                if not targid in groupedT:
                    groupedT[targid] = []
                if not targid in groupedP:
                    groupedP[targid] = []
                groupedT[targid].append(yt)
                groupedP[targid].append(yp)
                if not targid in targets:
                    targets[targid] = []
                targets[targid].append(ligid)
        ...
        for targID in targets:
            auc_roc = roc(groupedT[targID], groupedP[targID], f_toPrint, fig_filename, total_y_true, len(y_true) - total_y_true, colour[count])
            auc_pr = precision_recall(groupedT[targID], groupedP[targID], f_toPrint, fig_filename, colour[count])
            #enrichment(grouped, total_y_true)
            print(str(targID)+"\t"+str(np.sum(groupedT[targID]))+"\t"+str(auc_roc)+"\t"+str(auc_pr))
        count=count+1
