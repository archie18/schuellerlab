...
Returns:
	A loaded session in pymol with spheres for the atoms that are found matched in .cliques corresponding to their comparison is saved in a folder as ./code_lig
"""
import os, os.path
import sys
import argparse
...
def parser_(cliques, folder):
	...
	for i in cliques:
		...
		print 'first',first
		for line in open(folder+'/'+first,'r'):
			if len(line) > 65:
		...
		w = open(folder+'/'+first,'w')
		for x in pdb_temp1:
			w.write(x+'\n')
		w.close()
		print 'second',second
		for line in open(folder+'/'+second,'r'):
			if len(line) > 65:
...
def loadtopymol(pdbA, code, lig,folder):
	pymol= 'pymol ' 
	for i in pdbA:
		mv = 'mv '+folder+'/'+i+' '+folder+'/' #here we rename .pdbs for a better visualization in pymol gui
		pdb = i.split('-')
		new_pdb = pdb[0][0:4]+'-'+pdb[1][0:4]+'.pdb'
		mv += new_pdb
		os.system(mv)
		pymol = pymol+folder+'/'+ new_pdb + ' '
	return pymol+"-d 'select SEL, b>0.1;set sphere_scale, 0.20;show spheres, SEL;save "+folder+'/'+code+"_"+lig+".pse'" #the representations is set as spheres for all atoms found  matched in .cliques and save as code_lig.pse
...
def copyfiles(code1, class1, code2, class2, src_path, folder):
	"""
	...
	cp = 'cp ./' + src_path + pdb1 + ' ' + folder
	os.system(cp)
	cp = 'cp ./' + src_path + pdb2 + ' ' + folder
	os.system(cp)
	cp = 'cp ./' + src_path + clique + ' ' + folder
	os.system(cp)
...
def main():
	...
	for i in range(len(classif)-1):
			...
			if classif["PDB_code"][i] == args.code and classif["clss"][i] and classif["clss"][j] == args.lig:
				copyfiles(classif["PDB_code"][j], classif["clss"][j], classif["PDB_code"][i], classif["clss"][i], args.path + "/click_fold2/", folder)

			if classif["clss"][i] == args.lig and classif["PDB_code"][j] == args.code and classif["clss"][j] == args.lig:
				print copyfiles(classif["PDB_code"][i], classif["clss"][i], classif["PDB_code"][j], classif["clss"][j], args.path + "/click_fold1/", folder)

	pdb = get_filename(folder,pdbs)
	cliques = get_filename(folder,clique)
	print 'cliques:'
	for i in cliques:
		print i
	print 'pdbs:'
	for i in pdb:
		print i
	print 'parser:'
	parser_(cliques, folder)
	#loadtopymol(pdb)
	rm = 'rm *.pdb'
	os.system(rm)
	os.system(loadtopymol(pdb,args.code, args.lig,folder))
