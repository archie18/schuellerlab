...
# Main function
def pocket_compare(atomtypes, cutoff, cliquesize, dist, workdir, best=1, resume=False, progress, n_large):
	...
	#n_codes = 10 # Debug only
		...
		# Iterate over all codes
			...
			# Query pocket path
            		query_pocket = codes[i]+"_pocket_ph4_"+dist+".pdb" 
				...
				# Target pocket path
                		target_pocket = codes[j]+"_pocket_ph4_"+dist+".pdb"

				# Clique file name
				clique_filename1 = "%s/%s_pocket_ph4_%s-%s_pocket_ph4_%s.pdb.1.clique" % (output_path1, codes[i], dist, codes[j], dist) 
				clique_filename2 = "%s/%s_pocket_ph4_%s-%s_pocket_ph4_%s.pdb.1.clique" % (output_path2, codes[j], dist, codes[i], dist) 
    	...
    	sys.stdout.flush()
	sys.stderr.write("Finished in " + str(datetime.now() - startTime) + ", work percentage: "+ (progress/n_large)*100+"%\n")
