SELECT   td.chembl_id, md.chembl_id, MIN(pp.accession), MIN(pp.description), GROUP_CONCAT(do.source_domain_id), MIN(act.standard_value),AVG(act.standard_value) , MAX(act.standard_value), MIN(ms.synonyms), MIN(canonical_smiles),MIN(pp.organism), MAX(md.max_phase) -- por alguna razon esto arroja error 
FROM assays
...
LEFT JOIN molecule_synonyms as ms  ON act.molregno = ms.molregno -- datos ligando
     ...
     WHERE 1 
      ...
      -- AND td.target_type IN ("single protein", "protein complex")
       AND assays.confidence_score  > 8 -- esta linea deberia ser equivalente a la anterior ya que 4 es se sabe que el target es parte de un complejo de proteinas homologas, pero no lo es, con esto salen mas resultados
  ...
  --         (act.standard_relation IN ("<<","<")      AND act.standard_value <= 10000)
    --       )
       ...
       AND substring_index(trim(substring_index(substring_index(molfile,"\n",4),"\n",-1))," ",1) > 5  -- mas que 5 atomos pesados
