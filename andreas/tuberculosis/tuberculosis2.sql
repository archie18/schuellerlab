...
--    SELECT  assays.assay_id, td.tid, pp.accession, pp.description, do.source_domain_id, MIN(act.standard_value),AVG(act.standard_value) AS avg_val, MAX(act.standard_value), cp.molregno, md.chembl_id, ms.synonyms, canonical_smiles, pp.organism -- por alguna razon esto arroja error 
    SELECT   td.chembl_id, md.chembl_id, MIN(pp.accession), MIN(pp.description), MIN(do.source_domain_id), MIN(act.standard_value),AVG(act.standard_value) , MAX(act.standard_value), MIN(ms.synonyms), MIN(canonical_smiles),MIN(pp.organism), MIN(td.target_type) -- por alguna razon esto arroja error 
     FROM assays
...
LEFT JOIN molecule_synonyms as ms  ON act.molregno = ms.molregno -- datos ligando
     ...
     WHERE 1 
       AND td.organism LIKE ("Mycobacterium tuberculosis%")
      -- AND td.target_type IN ("single protein", "protein complex")
       AND tt.parent_type IN ('MOLECULAR', 'PROTEIN')
       AND assays.confidence_score  > 2 -- esta linea deberia ser equivalente a la anterior ya que 4 es se sabe que el target es parte de un complejo de proteinas homologas, pero no lo es, con esto salen mas resultados
