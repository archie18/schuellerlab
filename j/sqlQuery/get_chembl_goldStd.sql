...
--    SELECT  assays.assay_id, td.tid, pp.accession, pp.description, do.source_domain_id, MIN(act.standard_value),AVG(act.standard_value) AS avg_val, MAX(act.standard_value), cp.molregno, md.chembl_id, ms.synonyms, canonical_smiles, pp.organism -- por alguna razon esto arroja error 
    SELECT   td.chembl_id, md.chembl_id, MIN(pp.accession), MIN(pp.description), MIN(do.source_domain_id), MIN(act.standard_value),AVG(act.standard_value) , MAX(act.standard_value), MIN(ms.synonyms), MIN(canonical_smiles),MIN(pp.organism) -- por alguna razon esto arroja error 
     FROM assays
