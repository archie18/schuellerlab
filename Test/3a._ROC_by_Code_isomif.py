...
"""
Example:
python 3a._ROC_by_Code.py -i input_file
"""
...
for input_file in args.input:
	...
	classes = set(classes)
	#classes.remove("None")
	prot = list(table["code1"])
	prot.extend(list(table["code2"]))
	prot = set(prot)
	...
	for clss in prot:
		for metric in metrics:
			pred[metric][clss] = []
		true[clss] = []
		tbl1 = table.loc[table["code1"] == clss]
		tbl2 = table.loc[table["code2"] == clss]
		#print tbl1
		#print tbl2
	#sys.exit(1)
