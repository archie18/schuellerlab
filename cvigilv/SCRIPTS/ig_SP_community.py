...
def predictRelation(inputs):
    ...
    try:
        ...
        path_eigencentrality = [ML.vs[n]['eigencentrality'] for n in path]
    ...
    return ML.vs[source_node]['fold'],\
            ...
            '_'.join([str(n) for n in path]),\
            '_'.join([str(v) for v in path_edges]),\
            '_'.join([str(w) for w in path_weights]),\
            '_'.join([str(c) for c in path_communities]),\
            '_'.join([str(ev) for ev in path_eigencentrality])
...
if __name__ == '__main__':
    ...
    for fold in tqdm(range(10), total=10):
        ...
        # Calculate eigenvector centralities for graph
        ML.vs['eigencentrality'] = ML.evcent(directed=False,weights='weight')
        print("Calculated eigencentrality of fold")
