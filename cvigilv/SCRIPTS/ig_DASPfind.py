...
#
# Reimplementación de DASPfind (Ba-Alawi, 2016). Método predictivo
# de interacciones proteína-ligando basado en formalismo de redes.
# HISTORY:  0.0  CVV 2020.04.06   First version
# =============================================================
...
def predictRelation(inputs):
    ...
    try:
        ...
        for path in paths:
            ...
            path_weights = [1-ML.es[e]['weight'] for e in path_edges if ML.es[e]['weight'] != 1.0]
            path_score   = np.prod(path_weights)**(2.26*len(path_weights))
