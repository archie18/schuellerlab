...
# Global constants
CLICK   = '/CLUSTERFS/homes/rminho/Click_multiple_2017/click.ppc64el'
	...
	if matched_atoms_count == "0": # if no output or 0 atoms are matched
		...
		for line in cliquefile:
			if flag:
    				...
    				if line.split()[3] in AAGROUPS and line.split()[8] in AAGROUPS and line.split()[1] != num_residuo :
					# If same group
        				if AAGROUPS[line.split()[3]] == AAGROUPS[line.split()[8]]:
            					seq_similarity += 1
            					aascore08 += 1
            					aascore05 += 1
					# If not same group
...
def pocket_compare((atomtypes, cutoff, cliquesize, dist, d_Thr, workdir, mode, resume)):
	...
	print '-------------'
	print 'Atom_type:', cacb
	print 'Cutoff:', cutoff
	#print 'Nro_cliques:', cliquesize
	print 'd_Thr:', d_Thr
	...
	shutil.copy(actual_path+'/'+workdir+'/Output/'+out_path+'/click_fold1/Parameters.inp', actual_path+'/'+workdir+'/Output/'+out_path+'/click_fold2/Parameters.inp')
    	...
    	out = '%s/Output/%s/Parameters_%s_%s_%s_%s_%s_output1.out'%(workdir,out_path,cacb,cutoff,cliquesize,d_Thr, dist)
